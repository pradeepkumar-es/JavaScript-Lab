...
The trinary number system, also known as base-3, uses three digits -- 0, 1, and 2. It's similar to the familiar decimal (base-10) number system that we use every day, only it's based on powers of 3 instead of powers of 10. The right most position in a trinary number represents 3 to the power of 0, the next position to the left represents 3 to the power of 1, and so on.

Understanding the JavaScript parseInt method
Before we begin, it's important to understand that JavaScript provides a built-in function, parseInt, which can perform base conversion for us. For example, parseInt('102012', 3) would convert the base-3 number '102012' into its base-10 equivalent which is 302 . However, in this lab, to understand base conversion at a deeper level, we're going to ignore that this function exists and do the conversion ourselves.

Creating our Custom Conversion Function
We're going to write a function that takes a string representation of a trinary number as input (e.g. '102012'). If the input string is not a valid trinary number, the function will return 0.
...
export class Trinary {
	...
	toDecimal() {
		...
		for(let digit of this.trinaryNumber){
		...
		let decimalNumber = 0;
		let num = Number(this.trinaryNumber);
		let digit;
		let position = 0;
		while(num){
			digit = num % 10;
			decimalNumber += (digit * Math.pow(3, position))
...
console.log(firstNumber.toDecimal()); //302

const secondNumber = new Trinary("1211812");
console.log(secondNumber.toDecimal()); //0, invalid
